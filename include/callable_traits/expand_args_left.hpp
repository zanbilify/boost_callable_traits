...
* When `T` is a member function pointer, the aliased type is a `Container` template instantiation, where the first template type argument is a reference to the parent class of `T`, qualified according to the member qualifiers on `T`, such that this type is equivalent to `callable_traits::qualified_parent_class_of<T>`. The next type arguments passed to the `Container` template are the parameter list types of the member function. The supplied `RightArgs...` are passed as the right-most types in the template type argument list.
