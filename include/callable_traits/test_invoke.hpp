    ...
    namespace ctdetail {
        ...
        struct test_invoke<pmf<Pmf>, T, Args...> {
            ...
            substitution_success<decltype(
                (static_cast<U&&>(u).*p)(std::forward<Rgs>(rgs)...)
            )>;
            ...
            auto operator()(P&& p, U&& u, Rgs&&... rgs) const ->
            substitution_success<decltype(
                (std::declval<normalize_reference<typename std::remove_reference<U>::type>>().*p)
                    (std::forward<Rgs>(rgs)...)
            )>;
        ...
        struct test_invoke<F, Args...> {
            ...
            substitution_success<decltype(static_cast<T&&>(t)(std::forward<Rgs>(rgs)...))>;
