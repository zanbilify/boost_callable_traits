...
#include <functional>
#include <utility>
#include <callable_traits/is_cv_qualified.hpp>
...
void assert_cv_qualified() {
    CT_ASSERT(ct::is_cv_qualified<T>());
    CT_ASSERT(ct::is_cv_qualified<T&>());
    CT_ASSERT(ct::is_cv_qualified<T &&>());
    CT_ASSERT(ct::is_cv_qualified<T const>());
    CT_ASSERT(ct::is_cv_qualified<T const &>());
    CT_ASSERT(ct::is_cv_qualified<T const &&>());
    CT_ASSERT(ct::is_cv_qualified<T volatile>());
    CT_ASSERT(ct::is_cv_qualified<T volatile &>());
    CT_ASSERT(ct::is_cv_qualified<T volatile &&>());
    CT_ASSERT(ct::is_cv_qualified<T const volatile &>());
    CT_ASSERT(ct::is_cv_qualified<T const volatile &&>());

    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T&>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T &&>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T const>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T const &>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T const &&>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T volatile>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T volatile &>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T volatile &&>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T const volatile &>()))::value);
    CT_ASSERT(decltype(ct::is_cv_qualified(std::declval<T const volatile &&>()))::value);
}
...
int main() {
...

    {
        ...
        CT_ASSERT(!ct::is_cv_qualified<f>());
        CT_ASSERT(!ct::is_cv_qualified<l>());
        CT_ASSERT(!ct::is_cv_qualified<r>());
        CT_ASSERT(!ct::is_cv_qualified<c>());
        CT_ASSERT(!ct::is_cv_qualified<cl>());
        CT_ASSERT(!ct::is_cv_qualified<cr>());
        CT_ASSERT(!ct::is_cv_qualified<v>());
        CT_ASSERT(!ct::is_cv_qualified<vl>());
        CT_ASSERT(!ct::is_cv_qualified<vr>());
        CT_ASSERT(ct::is_cv_qualified<cv>());
        CT_ASSERT(ct::is_cv_qualified<cvl>());
        CT_ASSERT(ct::is_cv_qualified<cvr>());
    }
