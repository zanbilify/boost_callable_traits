...
void assert_rvalue_qualified() {
    ...
    CT_ASSERT(ct::is_rvalue_qualified<T>());
    CT_ASSERT(ct::is_rvalue_qualified<T&>());
    CT_ASSERT(ct::is_rvalue_qualified<T &&>());
    CT_ASSERT(ct::is_rvalue_qualified<T const>());
    CT_ASSERT(ct::is_rvalue_qualified<T const &>());
    CT_ASSERT(ct::is_rvalue_qualified<T const &&>());
    CT_ASSERT(ct::is_rvalue_qualified<T volatile>());
    CT_ASSERT(ct::is_rvalue_qualified<T volatile &>());
    CT_ASSERT(ct::is_rvalue_qualified<T volatile &&>());
    CT_ASSERT(ct::is_rvalue_qualified<T const volatile &>());
    CT_ASSERT(ct::is_rvalue_qualified<T const volatile &&>());

    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T&>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T &&>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T const>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T const &>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T const &&>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T volatile>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T volatile &>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T volatile &&>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T const volatile &>()))::value);
    CT_ASSERT(decltype(ct::is_rvalue_qualified(std::declval<T const volatile &&>()))::value);
}
